...
function buildAttemptRows(result) {
  const testLabel = result.testTitle || result.testId;
  const finishedAt = formatDate(result.finishedAt);
  return result.questionsReview.map((item, index) => {
    ...
    return {
      ...
      selected_answer: hasAnswer ? item.options[item.selectedIndex] : "Нет ответа",
